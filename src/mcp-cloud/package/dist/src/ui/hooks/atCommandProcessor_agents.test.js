...
import { FileDiscoveryService, GlobTool, ReadManyFilesTool, StandardFileSystemService, ToolRegistry, COMMON_IGNORE_PATTERNS, } from '@google/gemini-cli-core';
...
describe('handleAtCommand with Agents', () => {
    ...
    it('should detect agent reference and add nudge message', async () => {
        ...
        const nudgePart = parts.find((p) => typeof p === 'object' &&
            p !== null &&
            'text' in p &&
            p.text.includes('<system_note>'));
        expect(nudgePart).toBeDefined();
        if (nudgePart && typeof nudgePart === 'object' && 'text' in nudgePart) {
            expect(nudgePart.text).toContain('The user has explicitly selected the following agent(s): CodebaseInvestigator');
        }
    });
