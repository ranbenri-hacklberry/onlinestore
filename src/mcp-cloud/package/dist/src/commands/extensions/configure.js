...
async function configureSpecificSetting(extensionName, settingKey, scope) {
    ...
    await updateSetting(extensionConfig, extension.id, settingKey, promptForSetting, scope);
}
...
async function configureExtensionSettings(extensionConfig, extensionId, scope) {
    const currentScopedSettings = await getScopedEnvContents(extensionConfig, extensionId, scope);
    let workspaceSettings = {};
    if (scope === ExtensionSettingScope.USER) {
        workspaceSettings = await getScopedEnvContents(extensionConfig, extensionId, ExtensionSettingScope.WORKSPACE);
    }
    ...
    for (const setting of extensionConfig.settings) {
        ...
        await updateSetting(extensionConfig, extensionId, setting.envVar, promptForSetting, scope);
    }
}
