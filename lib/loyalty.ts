 ...
 */
export async function getLoyaltyCount(customerPhone: string | null, user: any) {
 ...
 */
export async function addCoffeePurchase(customerPhone: string | null, orderId: string, itemsCount = 1, user: any, redeemedCount = 0) {
  if (!customerPhone) {
 ...
 * @returns {Promise<{success: boolean, newCount: number, addedPoints: number}>}
 */
export async function handleLoyaltyAdjustment(customerPhone: string, orderId: string, pointsDelta: number, user: any, redeemedDelta = 0) {
  if (!customerPhone) {
    return { success: false, newCount: 0, addedPoints: 0 };
  }
 ...
 */
export async function getLoyaltyRedemptionForOrder(orderId: string, user: any) {
  if (!orderId) return 0;
  ...
  try {
    ...
    const totalPointsRedeemed = data.reduce((sum: number, tx: any) => sum + Math.abs(tx.change_amount), 0);
